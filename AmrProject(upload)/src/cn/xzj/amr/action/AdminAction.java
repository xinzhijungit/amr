...
import cn.xzj.amr.service.IAdminService;
...
public class AdminAction extends AbstractActionAdapter {
	...
	public ModelAndView list(HttpServletRequest request) {
		...
		try {
			Map<String,Object> map = this.adminService.list(super.getEid(request),split.getColumn(), split.getKeyword(), split.getCurrentPage(), split.getLineSize()) ;
			mav.addObject("allEmps", map.get("allEmps")) ;
			split.setAttribute(request, map.get("empCount"), "admin.list.action", null, null, null, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (super.isAuth(2, request)) {
			...
			try {
				if (this.adminService.add(vo, super.getEid(request))) {
					...
					super.setMsgAndUrl("vo.add.success", "admin.add.action", mav);
				} else {
